...
| 기능 | 설명 |
|---|---|
| 자동 출근/퇴근 | 음성채널 **화면 공유**를 감지해 (공유 중 시간만 기록) `🌅 홍길동님 오전 출근 (09:02)` 형식으로 공지 |
| 출석 정산 | 오전(09\~12) / 저녁(19\~22) 시간대 내 **누적 1시간 이상** 참여 시 출석 인정 (하루 1회) |
...
```
📌 정보      환영-인사 · 공지사항 · 명령어-가이드   (읽기전용, 봇이 안내 게시)
📖스터디     모각코(음성, 출석 감지) · 명예의-전당(랭킹·레벨업) · 출석-알림(봇 전용 로그) · 스터디-대화방
☕ 자유      잡담 · 공부방 1 · 공부방 2
역할        운영진 / 스터디원(입장 시 자동) / 출석유예 / Lv.1~5 레벨 역할(자동)
```
...
│   ├── ranking.js      # 주간·월간 출석 랭킹 (공동 순위 지원)
│   ├── levels.js       # 누적 출석 레벨, 역할 자동 부여
│   ├── scheduler.js    # cron 등록
│   └── commands.js     # /출석현황, /유예, /유예해제 슬래시 명령어
